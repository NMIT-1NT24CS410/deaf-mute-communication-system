...
def load_sign_image(char):
    """Load sign image for a character, return placeholder if not found."""
    ...
    # Placeholder: dark background with letter
    placeholder = np.zeros((WIN_H, WIN_W, 3), dtype=np.uint8)
    placeholder[:] = (30, 30, 50)
    cv2.putText(placeholder, char, (WIN_W//2 - 80, WIN_H//2 + 60),
                cv2.FONT_HERSHEY_SIMPLEX, 8, (100, 220, 255), 12)
...
def display_signs(text):
    """Display ISL sign images for each character in text."""
    ...
    if not chars:
        print("[INFO] No alphanumeric characters to display.")
        return

    print(f"\n[DISPLAY] Showing {len(chars)} signs for: '{text}'")
    cv2.namedWindow("ISL Sign Display", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ISL Sign Display", WIN_W, WIN_H)

    for i, char in enumerate(chars):
        img = load_sign_image(char)
        ...
        cv2.putText(img, f"ISL Sign: {char}", (20, 45),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.3, (0, 255, 150), 3)
        cv2.putText(img, f"{i+1} / {len(chars)}", (WIN_W - 120, 45),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (200, 200, 200), 2)

        progress = int((i + 1) / len(chars) * WIN_W)
